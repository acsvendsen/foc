...
def _velocity_point_to_point_to_target(
    odrv,
    axis,
    target,
    *,
...
def run_velocity_point_to_point_move(
    *,
    odrv=None,
    axis=None,
    serial_number=None,
    ...
    if delta_turns is None and target_turns is None:
        raise ValueError("run_velocity_point_to_point_move requires delta_turns or target_turns")
    candidate_override = dict(candidate_override or {})
    ...
    stages = []
    travel = _velocity_point_to_point_to_target(
        odrv,
        axis,
        target,
        timeout_s=float(timeout_s),
    ...
    if bool(ok) and bool(return_to_start):
        ...
        result["return_pos"] = float(getattr(axis.encoder, "pos_estimate", start_pos))
        result["return_residual"] = float(result["return_pos"] - float(start_pos))
        if ret["ok"] and float(return_hold_s) > 0.0:
            _apply_position_loop_config(axis, **final_loop_cfg)
            ret_hold = _move_and_observe(
                odrv,
                axis,
                start_pos,
            ...
            ret_hold["stage"] = "return_hold"
            result.setdefault("stages", []).append(ret_hold)
